...
"""
    ...
    with file_path.open('r', encoding='utf-8') as file:
        ...
        if first_line.strip().startswith('#'):
            ...
            tags = [tag.strip('#') for tag in possible_tags if tag.startswith('#') and len(tag.strip('#')) > 0]
            print(f"tags: {tags}")
    ...
    authors_formatted = ', '.join(author_names) if isinstance(author_names, (list, tuple)) else author_names
    if tags:
        tags_formatted = '\n  - '.join(tags)
...
def header_conversion(authors: str | List[str], directory: str | None, file_path: str | None):
    if file_path:
